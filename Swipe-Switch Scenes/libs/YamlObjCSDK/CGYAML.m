...
//			}
//            
//			[self paragraphs:value buffer:buffer];
...
- (NSString *)paragraphs
{
	...
	for (id rootNode in [self documents]) {
        [self paragraphs:rootNode buffer:descrBuffer];
	}
    return descrBuffer;
}

- (void)pageNum:(id)anObject buffer:(NSMutableString *)buffer
{
    if ([anObject isYAMLMappingNode]) {
		for (id key in [anObject allKeys]) {
			id value = [anObject objectForKey:key];
            if ([value isYAMLScalarNode]) {
                NSString *castedKey = key;
                NSString *valueToCheckFor = @"number";
                
                if ([castedKey isEqualToString:valueToCheckFor]) {
                    // this outputs our values we need!
                    //[buffer appendFormat:@"YAMLMappingNode %@%@: %@\n", [self indentString:0], key, value];
                    [buffer appendFormat:@"%@", value];
                }                
           ...
           // These are our directories
           // [buffer appendFormat:@"%@%@:\n", [self indentString:0], key];
			[self pageNum:value buffer:buffer];
		}				
		return;
	}
}
...
- (NSString *)pageNum
{
    NSMutableString *descrBuffer = [NSMutableString string];
	for (id rootNode in [self documents]) {
        [self pageNum:rootNode buffer:descrBuffer];
	}
    return descrBuffer;
}
...
- (void)description:(id)anObject indent:(int)indent buffer:(NSMutableString *)buffer
{
	...
	// key: object name in yaml file i.e., line1text
    // value: key's value, so line1texts value should be a sentence
	if ([anObject isYAMLMappingNode]) {
		for (id key in [anObject allKeys]) {
			id value = [anObject objectForKey:key];
            if ([value isYAMLScalarNode]) {
...
- (void)pageNumber:(id)anObject indent:(int)indent buffer:(NSMutableString *)buffer
{
	if ([anObject isYAMLScalarNode]) {
		[buffer appendFormat:@"%@%@", [self indentString:indent], anObject];
		return;
